...
const SidebarContent = ({ collapsed, links, userProfile, userRole, handleLogout, setCollapsed }) => (
    <div className="flex flex-col h-full bg-slate-900 text-slate-300">
        ...
        <div className={`flex items-center ${collapsed ? "justify-center" : "gap-3"} px-5 py-6 border-b border-slate-800 relative overflow-hidden group`}>
            <div className="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center flex-shrink-0 relative z-10 transition-transform group-hover:scale-105">
                <span className="text-white font-bold text-sm tracking-tight text-center">BCTA</span>
            </div>
            {!collapsed && (
                <div className="relative z-10">
                    <p className="font-bold text-white text-sm leading-tight tracking-tight">Management System</p>
                    <p className="text-[11px] font-medium text-slate-400 leading-tight tracking-wider uppercase">Bhimavaram</p>
                </div>
            )}
        </div>
        ...
        {/* User Mini Profile */}
        <div className={`p-4 border-t border-slate-800 bg-slate-900 transition-all ${collapsed ? "flex flex-col items-center gap-3" : ""}`}>
            {!collapsed ? (
                <div className="flex items-center gap-3 bg-slate-800 rounded-lg p-3 mb-3 hover:bg-slate-700 transition-colors">
                    {userProfile?.photoURL ? (
                        <img src={userProfile.photoURL} alt="" className="w-10 h-10 rounded-md object-cover" />
                    ) : (
                ...
                <div className="w-10 h-10 rounded-md bg-slate-800 flex items-center justify-center mb-2" title={userProfile?.name}>
                    {userProfile?.name?.[0] || "U"}
                </div>
            )}
            ...
            <div className={`flex ${collapsed ? "flex-col" : "gap-2"}`}>
                ...
                <button
                    ...
                    className={`flex items-center justify-center p-2 rounded-md text-slate-400 hover:text-white hover:bg-red-500/80 transition-colors focus:outline-none ${!collapsed ? "flex-1 gap-2" : "w-full"}`}
                    title="Logout"
                >
                    <LogOut size={18} />
