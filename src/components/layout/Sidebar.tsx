...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const memberLinks = [
    { to: "/member/dashboard", icon: LayoutDashboard, label: t("sidebar.dashboard") },
    { to: "/member/profile", icon: User, label: t("sidebar.myProfile") },
    { to: "/member/meetings", icon: CalendarDays, label: t("sidebar.myMeetings") },
    { to: "/member/scan", icon: Smartphone, label: t("sidebar.scanQR") },
    { to: "/member/payments", icon: CreditCard, label: t("sidebar.myPayments") },
    { to: "/member/notifications", icon: Bell, label: t("sidebar.notifications") },
    {
      to: "/member/complaint",
      icon: MessageSquareWarning,
      label: t("sidebar.raiseComplaint"),
    },
    { to: "/member/emergency", icon: Stethoscope, label: t("sidebar.emergency") },
  ];
  ...
  const handleLogout = useCallback(async () => {
    try {
      toast.loading(t("common.loading"), { id: "logout-toast" });
      await logout();
      ...
      setMobileOpen(false);
      toast.success(t("topbar.loggedOut"), { id: "logout-toast" });
      navigate("/login", { replace: true });
    } catch (error) {
      toast.error(t("topbar.logoutFailed"), { id: "logout-toast" });
    }
  }, [logout, setMobileOpen, navigate, t]);
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{
          background: "linear-gradient(180deg, #02052c 0%, #052a49 100%)",
        }}
      >
        <SidebarContent
          collapsed={collapsed}
      ...
      <div className="flex flex-col h-full text-slate-300">
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse at 30% 50%, rgba(99,102,241,0.15) 0%, transparent 70%)",
            }}
          />
          <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 relative z-10 transition-transform duration-300 hover:scale-105 shadow-lg">
            <img
              src={assets.herologo}
              alt="Hero Logo"
          ...
          {!collapsed ? (
            ...
            >
              ...
              <div className="overflow-hidden flex-1">
                ...
                <p className="text-xs text-indigo-300/60 capitalize font-medium">
                  {userRole}
                </p>
              </div>
            </div>
