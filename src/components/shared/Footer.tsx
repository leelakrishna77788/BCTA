...
const Footer: React.FC = () => {
  return (
    <footer
      id="footer"
      className="relative bg-slate-950 text-white px-5 sm:px-8 py-16 sm:py-20 overflow-hidden"
    >
      {/* Gradient top accent */}
      <div
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 sm:gap-8 pb-16 border-b border-white/5">
          ...
          <div className="flex flex-col items-center sm:items-start text-center sm:text-left space-y-6">
            ...
            <p className="text-sm text-slate-400 leading-relaxed max-w-xs">
              Empowering Bhimavaram's technicians with digital unity, real-time
              communication, and smart association management tools.
            </p>
            ...
            <div className="flex items-center gap-4 pt-2">
              {[
                ...
                <a
                  key={i}
                  href={social.href}
                  className={`w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-slate-400 transition-all duration-300 ${social.color} hover:bg-white/10 hover:-translate-y-1`}
                >
                  <social.icon size={18} />
                </a>
              ))}
            ...
            <div className="space-y-4">
              {[
                ...
                <Link
                  key={link.label}
                  to={link.to}
                  className="block text-slate-400 hover:text-white font-medium text-[15px] transition-all duration-200 hover:translate-x-1"
                >
                  {link.label}
                </Link>
              ))}
            ...
            <div className="space-y-6">
              ...
              <div className="pt-4 space-y-3 border-t border-white/5 w-full">
                <p className="text-slate-500 text-[11px] font-bold uppercase tracking-widest flex items-center gap-2 justify-center sm:justify-start">
                  <MapPin size={12} className="text-indigo-500" /> Bhimavaram,
                  AP
                </p>
