...
import type { UserRole } from "../../types/member.types";
...
const ProtectedRoute = memo(({ children, allowedRoles }: ProtectedRouteProps) => {
  ...
  if (allowedRoles && !isAllowed) {
    console.warn(
      `[ProtectedRoute] Access denied for role '${normalizedRole}'. Redirecting...`
    );
    if (!normalizedRole) return <Navigate to="/login" replace />;

    if (normalizedRole === "member") return <Navigate to="/member/dashboard" replace />;

    const isAdmin = ADMIN_ROLES.map((r) => r.toLowerCase().trim()).includes(
