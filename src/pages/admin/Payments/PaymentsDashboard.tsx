    ...
    return (
        <div className="space-y-5 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                {[
                    { label: "Total Value", value: `₹${totalRevenue.toLocaleString()}`, color: "bg-[#000080]", icon: CreditCard },
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filtered.map(p => (
                                <tr key={p.id} className={`hover:bg-slate-50 transition-colors ${parseFloat(p.remainingAmount as string) > 0 ? "border-l-4 border-red-500" : ""}`}>
