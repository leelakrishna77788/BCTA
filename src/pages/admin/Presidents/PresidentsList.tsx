...
const PresidentsList: React.FC = () => {
  ...
  const [deletingPresident, setDeletingPresident] =
    useState<President | null>(null);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deletePresident(
        deletingPresident.id,
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-24 bg-white/60 rounded-3xl border border-slate-200/60 animate-pulse"
            />
          ))}
        </div>
      ) : presidents.length === 0 ? (
        ...
        <div className="space-y-3">
          {presidents.map((p, i) => (
            <PresidentCard
              ...
              index={i}
              onEdit={(id) =>
                navigate(`/admin/presidents/${id}/edit`)
              }
