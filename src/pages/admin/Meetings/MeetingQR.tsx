...
const MeetingQR: React.FC = () => {
    ...
    useEffect(() => {
        if (isActive && id) {
            rotationTimer.current = setInterval(() => {
                ...
                setCountdown(prev => {
                    if (prev <= 1) {
                        const duration = meeting?.qrDuration ?? 30;
                        updateDoc(doc(db, "meetings", id), {
                            qrToken: generateId(),
                            qrExpiresAt: Timestamp.fromDate(new Date(Date.now() + duration * 60 * 1000))
                        }).catch(console.error);
                        return 30;
                    }
                    return prev - 1;
    ...
    }, [isActive, id, meeting?.qrDuration]);
                        ...
                        {!isActive ? (
                            ...
                            <div className="text-center space-y-6">
                                <div className="relative p-6 border-2 border-slate-100 rounded-3xl bg-white shadow-xl">
                                    ...
                                    <div className="absolute -top-3 -right-3 w-12 h-12 bg-[#000080] rounded-2xl flex items-center justify-center text-white font-bold">{countdown}s</div>
                                </div>
                                <button onClick={stopAttendance} className="btn-danger w-full py-3 rounded-lg flex items-center justify-center">Stop Session</button>
                            </div>
                        )}
                    </div>
